...
本项目 Fork 自 [yaobiao131/downkyicore](https://github.com/yaobiao131/downkyicore)
...
- 收藏夹自动监控下载功能：定时轮询指定收藏夹，检测到新视频后自动解析并加入下载队列
- 支持热更新：启用/禁用、轮询间隔、收藏夹ID、保存目录、下载内容等设置项均可实时生效，无需重启
- 新增设置页「自动监控」标签页，支持配置收藏夹ID、轮询间隔（10/30/60分钟）、下载目录、下载内容
- 自动监控支持去重防护：SQLite 本地记录 + 下载队列重复检查，双重保障
- 文件命名格式新增「下载时间」字段，支持按下载日期组织目录结构（如 `2026-05-30/视频标题.mp4`）

### 变更

- 项目目标框架从 .NET 6.0 升级至 .NET 8.0
- 轮询间隔最短限制为 10 分钟（防止 B站 API 限流）

### 开发者

- 新增 `downkyi.code-workspace` VS Code 工作区文件

---

## 上游变更历史

以下为上游项目 [yaobiao131/downkyicore](https://github.com/yaobiao131/downkyicore) 的变更记录。
